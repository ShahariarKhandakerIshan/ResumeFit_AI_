document.addEventListener('DOMContentLoaded', () => {
  ...
  const populateResults = (data) => {
    ...
    const roles       = data.advanced_features_enabled ? (data.predicted_roles || []) : [];
    const isDemoMode  = !data.advanced_features_enabled;

    const diffWorkspace = document.querySelector('.diff-workspace');
    const commitsDrawer = document.querySelector('.commits-drawer');
    if (diffWorkspace) diffWorkspace.style.display = isDemoMode ? 'none' : '';
    if (commitsDrawer) commitsDrawer.style.display = isDemoMode ? 'none' : '';
  ...
  const resetToCommandCenter = () => {
    ...
    const diffWorkspace = document.querySelector('.diff-workspace');
    const commitsDrawer = document.querySelector('.commits-drawer');
    if (diffWorkspace) diffWorkspace.style.display = '';
    if (commitsDrawer) commitsDrawer.style.display = '';
